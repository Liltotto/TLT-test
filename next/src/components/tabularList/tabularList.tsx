...
export default function TabularList({products, setLimit}: {products: ResultProduct[], setLimit: (limit: number) => void} ) {
        ...
        <table className="min-w-full bg-transparent border-separate border-spacing-y-2.5">
          ...
          <tbody className="[&>*:nth-child(odd)]:bg-transparent [&>*:nth-child(even)]:bg-[#0F172A08]">
            {products?.map((product: Product) => (
              <tr key={product.id} className="h-[80px] text-[13px] font-normal leading-[16px] tracking-[0%] text-center  ">
                ...
                <td className="py-2.5 px-[38.6px] whitespace-nowrap ">{product.name}</td>
                <td className="py-2.5 px-[38.6px] whitespace-nowrap ">
                  {product.quantity}
                </td>
                <td className="py-2.5 px-[38.6px] whitespace-nowrap ">Ламповый завод</td>
                <td className="py-2.5 px-[38.6px] whitespace-nowrap ">{product.price} р</td>
